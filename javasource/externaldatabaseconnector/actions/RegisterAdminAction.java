// This file was generated by Mendix Studio Pro.
//
// WARNING: Only the following code will be retained when actions are regenerated:
// - the import list
// - the code between BEGIN USER CODE and END USER CODE
// - the code between BEGIN EXTRA CODE and END EXTRA CODE
// Other code you write will be lost the next time you deploy the project.
// Special characters, e.g., é, ö, à, etc. are supported in comments.

package externaldatabaseconnector.actions;

import com.mendix.core.Core;
import com.mendix.m2ee.api.IAppContainer;
import com.mendix.systemwideinterfaces.core.IContext;
import com.mendix.webui.CustomJavaAction;
import externaldatabaseconnector.livepreview.actions.MxExecuteCallableAdminAction;
import externaldatabaseconnector.livepreview.actions.MxExecuteQueryAdminAction;
import externaldatabaseconnector.livepreview.actions.MxExecuteStatementAdminAction;
import externaldatabaseconnector.livepreview.actions.MxGetSchemaInfoAdminAction;
import externaldatabaseconnector.livepreview.actions.MxTestConnectionAdminAction;
import externaldatabaseconnector.livepreview.actions.MxCheckAdminAction;
import externaldatabaseconnector.proxies.constants.Constants;

public class RegisterAdminAction extends CustomJavaAction<java.lang.Void>
{
	public RegisterAdminAction(IContext context)
	{
		super(context);
	}

	@java.lang.Override
	public java.lang.Void executeAction() throws Exception
	{
		// BEGIN USER CODE
		IAppContainer context = getContext().getRuntimeRequest().get().getContext();
		context.registerAdminAction(new MxCheckAdminAction());
		context.registerAdminAction(new MxTestConnectionAdminAction());
		context.registerAdminAction(new MxTestConnectionAdminAction());
		context.registerAdminAction(new MxGetSchemaInfoAdminAction());
		context.registerAdminAction(new MxExecuteQueryAdminAction());
		context.registerAdminAction(new MxExecuteStatementAdminAction());
		context.registerAdminAction(new MxExecuteCallableAdminAction());
		Core.getLogger(Constants.getLogNode()).info("EDC: LivePreview initialized successfully.");

		return null;
		// END USER CODE
	}

	/**
	 * Returns a string representation of this action
	 * @return a string representation of this action
	 */
	@java.lang.Override
	public java.lang.String toString()
	{
		return "RegisterAdminAction";
	}

	// BEGIN EXTRA CODE
	// END EXTRA CODE
}
